...
ドメインは小文字の英数字とハイフンをドットで区切る。各ラベルは 1〜63 文字で、
先頭・末尾のハイフンは禁止。ドメイン全体は 253 文字以内とする。
...
サービス停止に失敗した場合は後続の削除を行わない。resolver や CA の信頼解除に
失敗した場合も再試行に必要な状態を残す。表示されたエラーを解消して同じコマンドを再実行する。
...
Webの両方で確認できる。画面は1秒ごとに更新され、一時停止・再開と
自動スクロールに対応する。
...
そのため、`localapp add` で登録したサービスからは Web ログは配信されない。
Web ログが必要な場合は、対象プロセスを `localapp run` で起動し直す。
ログファイルや別プロセスの出力を localapp に流し込むコマンドは現時点ではない。

ログは受信時刻を基準に直近5分間のみ、各サービス最大256 KiB、最大64サービス分をメモリに保持し、
